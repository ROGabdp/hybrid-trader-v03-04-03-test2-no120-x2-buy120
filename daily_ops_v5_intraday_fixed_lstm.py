...
"""
================================================================================
Daily Operations - V5 Intraday Fixed LSTM (No Filter Mode)
================================================================================
盤中執行腳本 - 使用 V5 模型 (對稱獎勵訓練) + 固定 LSTM 模型
...
**Fixed LSTM 特點**:
- 不進行每日重訓，結果完全可重現
- 讀取 backtest_v5_dca_hybrid_no_filter_fixed_lstm.py 產生的 lstm_info.json
...
4. 輸出結果到 intraday_runs_v5_fixed/{date}_{time}/
...
INTRADAY_RUNS_PATH = os.path.join(PROJECT_PATH, 'intraday_runs_v5_fixed')  # V5 Fixed LSTM 專屬資料夾
...
def load_lstm_info_from_backtest(csv_file: str = None) -> dict:
    ...
    if not json_files:
        ...
        print(f"        請先執行: python backtest_v5_dca_hybrid_no_filter_fixed_lstm.py --start 2025-01-02")
        return None
...
def v5_inference(workspace: dict, df: pd.DataFrame, open_positions: list = None, current_price: float = None) -> dict:
    ...
    print(f"  [濾網] Signal_Buy_Filter = {signal_buy_filter} (僅供參考，V5不受限)")
    ...
    if not os.path.exists(buy_path):
        return {'error': 'V5 Model not found', 'filter_status': signal_buy_filter}
...
def generate_intraday_report(workspace: dict, df: pd.DataFrame, res: dict, date_str: str, intraday_data: tuple, avg_volume: float, backtest_status: dict = None):
    ...
    lines = []
    lines.append("=" * 50)
    lines.append(f"📅 V5 盤中即時分析 - {date_str}")
    ...
    lines.append(f"📋 [濾網狀態] {filter_icon} {'通過' if filter_status else '未通過'} (V5不受此限制)")
    lines.append("-" * 50)
    ...
    # 額外標注濾網狀態供參考
    filter_note = "濾網✅" if filter_status else "濾網❌"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='V5 盤中分析系統 (Fixed LSTM + No Filter)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
使用範例:
  python daily_ops_v5_intraday_fixed_lstm.py                    # 使用最新回測結果
  python daily_ops_v5_intraday_fixed_lstm.py -i                 # 互動式選擇回測 CSV
  python daily_ops_v5_intraday_fixed_lstm.py --backtest-start 2025-01-02  # 指定回測起始日
        '''
    )
    parser.add_argument(
        '-i', '--interactive',
    ...
    print(f"🚀 V5 盤中分析系統 (Fixed LSTM + No Filter) - {date_str} {time_str}")
