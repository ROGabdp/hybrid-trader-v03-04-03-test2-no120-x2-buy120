...
def train_lstm_models():
    ...
    # =========================================================================
    print("\n" + "-" * 60)
    print("📊 [1/3] Training LSTM T+1 Model (Multivariate)")
    print("-" * 60)
    ...
    # =========================================================================
    print("\n" + "-" * 60)
    print("📈 [2/3] Training LSTM T+5 Model (5-Day Direct)")
    print("-" * 60)
    ...
    # =========================================================================
    # 3. 訓練 T+20 模型 (20-Day Direct)
    # =========================================================================
    print("\n" + "-" * 60)
    print("📈 [3/3] Training LSTM T+20 Model (20-Day Direct)")
    print("-" * 60)
    
    cmd_20d = [
        sys.executable,
        "twii_model_registry_20d.py",
        "train",
        "--start", TRAIN_START,
        "--end", TRAIN_END
    ]
    
    print(f"[Command] {' '.join(cmd_20d)}")
    result_20d = subprocess.run(cmd_20d, cwd=PROJECT_PATH)
    
    if result_20d.returncode != 0:
        print(f"[Error] T+20 model training failed with code {result_20d.returncode}")
    else:
        print("[Success] T+20 model training completed!")
    ...
    # T+20 模型路徑
    models_20d_dir = PROJECT_PATH / "saved_models_20d"
    if models_20d_dir.exists():
        models_20d = list(models_20d_dir.glob(f"model_{TRAIN_START}_{TRAIN_END}.keras"))
        if models_20d:
            print(f"\n✅ T+20 Model Artifacts:")
            print(f"   {models_20d_dir / f'model_{TRAIN_START}_{TRAIN_END}.keras'}")
            print(f"   {models_20d_dir / f'feature_scaler_{TRAIN_START}_{TRAIN_END}.pkl'}")
            print(f"   {models_20d_dir / f'target_scaler_{TRAIN_START}_{TRAIN_END}.pkl'}")
            print(f"   {models_20d_dir / f'meta_{TRAIN_START}_{TRAIN_END}.json'}")
    
    print("\n" + "=" * 70)
    print("  ✅ Phase 2.5 Completed: Long-Period LSTM Models Trained (T+1, T+5, T+20)")
    print("=" * 70 + "\n")
